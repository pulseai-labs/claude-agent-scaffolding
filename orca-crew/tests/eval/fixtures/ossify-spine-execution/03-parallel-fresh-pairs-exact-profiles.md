...
expected_reason: 'THE DISCRIMINATING FIXTURE for this surface. Each item gets its
  own FRESH implementer terminal launched from that items SEATS row, verbatim, using
  ...
  launches by seat name. TWO terminals for two items at this point, not four: the verifier
...
not have. A `strong-coder` implementer terminal from an earlier, unrelated
