...
**`blocked_patterns contains an empty-string entry or an entry with a line break`.** An empty pattern has no coherent meaning — as a regex it would match every message — so the filter refuses to guess; a pattern containing a line break cannot be applied as one pattern either. Edit the manifest and remove or fix that entry. To allow every commit deliberately, set `blocked_patterns` to `[]` or `enforce` to `false` instead. No hook reinstall is needed.
