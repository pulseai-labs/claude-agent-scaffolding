...
- **Review round 1 — every block names a repair that fixes that exact condition.** The hook bakes the `wi` dispatcher path (`__WI_BIN_PATH__`) and prints `%q`-quoted absolute paths; corrupt/missing manifests name `wi manifest_write` (the verb that actually rewrites `pairing.json`), unreachable baked paths name the moved-workspace re-bake forms, and the canonical root is resolved via `git rev-parse --git-common-dir` so a commit from a linked worktree names the main worktree — the only path the installer accepts. `wi_trace_filter_install` canonicalises both roots to absolute physical paths, so a relative hand-typed repair works, and `wi_trace_filter_install_pair` repairs the load-bearing canonical hook first, still reporting (and returning non-zero on) an AI-side refusal.
