...
never positional `$1`/`$2` per the slash-command `$N` substitution bug; on
shim-less channels like Devin nothing exports `$ARGUMENTS`, so read the values
as literal tokens in the invocation/request text):
