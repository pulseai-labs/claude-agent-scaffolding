...
- id: system-prompt
  config:
    ...
    personaPrefix: >-
      ...
      handoff's `base_branch` from the existing handoffs' `base_branch:` lines, never from
      HEAD. If the recorded state does not reconcile cleanly, stop and ask the operator
      before any mutation. When the lane has ended and the operator or orchestrator asks
