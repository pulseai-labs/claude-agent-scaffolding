...
expected_reason: 'Who executes the merge is the top''s explicit assignment,
  ...
  the reviewer and PR-fix seats are released when their work finishes, as
  MECHANICS''s Teardown says with the receipt read and `herdr workspace list`
  confirming — and are exempt from the record-pass hold — while the session deletes no branch:
