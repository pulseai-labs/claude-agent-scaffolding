...
expected_reason: 'All four activation facts hold, so the phase applies. The top agrees
  ...
  starts EXACTLY ONE spine session - it launches no item pane itself - injecting
  ...
  a failure halts and asks, never substitutes - then creates a `run.json` of its own
  ...
  top launching the item panes because it is the session holding the approved
  seats;
  ...
  task and reads that seat''s report file for what it opened - close is a dispatched
  command, not one the top
  runs in its own session'
---

You are the orchestrator session. A run's `run.json` is bound for the objective, this session
...
first round begins executing. Name every seat you create and every dispatch you
send, and say who creates it. Say what you check before starting anything, and
what you do about nested worker depth, which no herdr command reads back.
