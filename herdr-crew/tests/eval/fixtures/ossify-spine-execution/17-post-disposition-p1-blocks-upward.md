...
expected_reason: 'A finding that arrives after the disposition ledger exists returns
  through the blocking question to the top BEFORE any seat acts on it - a post-disposition P1
  ...
  the reasoning the blocking question to the top exists to stop: a post-disposition finding changes
  the disposition, and the disposition is the record both the ledger and the merge
  gate read. So the proposal is refused: the question goes up, the seats wait, and
