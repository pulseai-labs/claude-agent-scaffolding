...
expected_reason: 'NEGATIVE CASE. The SEATS block is launch authority, and a seat
  ...
  step-1 check of the block against SPINE.md fails here before any pane is
...
You are the spine session for `r9.s2` ("notification rules"), with the `run.json`
you own. Your brief carried the SEATS block the top injected, and before launching
