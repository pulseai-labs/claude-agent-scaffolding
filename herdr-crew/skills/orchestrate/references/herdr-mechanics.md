...
1. **The run's workspace, once per run.**
   ...
   Its response already holds one tab and one shell pane (`.result.tab`,
   `.result.root_pane`), and they are the first seat's: label the tab with
   `herdr tab rename <tab> "seat: <role> (<agent>)"` rather than creating a second tab
   beside an empty one.
2. **One tab per further seat.**
   `herdr tab create --workspace <id> --cwd <the seat's tree> --label "seat: <role> (<agent>)" --no-focus`.
...
5. **The model.** `herdr pane read <pane>`: the default source for `model_shows: banner`,
   `--source visible` (the rendered viewport) for `screen`. The pane must show
   `expected_model:`. A pane showing `command not found` or another model is a failed
   launch (`roles.md`).
...
`herdr pane wait-output <pane> --regex '<pattern>' --timeout <ms>`, or `--match '<text>'`
for a literal substring. The pattern is what the agent shows once it takes input, and its
machine entry's `note:` records it. An entry that gives none is asked of the operator,
never guessed. `--timeout` is not optional: without it the wait never ends. The wait
searches what is already on screen before it polls, so a pattern that the launch line
itself matches proves nothing.
...
`HERDR_WORKSPACE_ID`), never prompt text. `brief_delivery: inject` prompts the brief
itself. `file` writes the brief to an absolute path the seat can read and prompts one line
pointing there.
...
- `agent_blocked`: the seat was already at a dialog and nothing was sent. Handle it as a
  `blocked` wake.
- `blocked`: the turn opened on a dialog. Handle it as a `blocked` wake.
...
This surface has no turn-start check and no doorbell (see Completion).
...
Before each dispatch, note whether the file exists and its hash (`git hash-object <path>`).
The wake is the typed wait above. After it:
...
An undetected seat has no doorbell. After dispatch the orchestrator returns idle and looks
for the report at its `REPORT_PATH` on a later turn. A screen poll in its place would be
the loop of waits that `lifecycle.md` forbids.
...
sibling pane split in the current tab. A seat reading that guide does not split. Five
seats are five tabs, each readable at full width, with herdr's sidebar as the index. The
...
A lane's provider variables are invoked by name through `command:`, never replayed with
`--env`: a replayed environment is the silent-reroute defect.
