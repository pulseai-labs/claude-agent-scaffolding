...
[ -n "$tag_repos" ] \
  || { echo "close: $rel has no repo to tag - its closed spines hold no non-abandoned work item, so there is no landed line to publish. A withdrawal made by mistake returns with \"$oss_bin\" work_item_status <wi-id> planned - but that ALONE re-tags a repo whose work never landed: the selector above takes any non-abandoned item whose spine is closed, and this one's spine is closed by construction here, so a planned item is exactly what it wants. Nothing else stops it either - work_item_status keeps no parent-spine guard. Reopen and land it first (\"$oss_bin\" spine_status <spine-id> active, that item through its round, the spine closed again), then re-run this close; a release with nothing to publish is the operator's decision (there is no release-retirement status), so this ceremony stops here rather than deciding for them - halt"; exit 1; }
