...
- **Promote rejected instinct-recurrence fingerprints (#483 R1/R2).** The unknown-fingerprint precheck only consulted `candidate_promotions[]` and `principle_promotions[]`, so every instinct-only candidate — surfaced from `recent_runs[].instinct_observations[]` by `ac_promotion_instinct_signal` — was refused. The precheck now covers all three promotable sources, and the promote jq program takes its text from the source that surfaced the fingerprint (`texts[0]` for candidates, the observation itself for instincts) via `first(...)`, with an `error()` backstop so the program still cannot emit a non-object on a miss. The same precheck no longer conflates "fingerprint absent" (jq exit 1) with "state.json missing, unreadable, or corrupt" (exit ≥ 2): the latter preserves jq's stderr and reports a state problem.
- **A funnel refusal could leak `state.lock` (#483 C1).** Under `bin/arc`'s `set -euo pipefail`, a caller that invoked `ac_guarded_jq_write` as a bare command aborted before reaching its own `ac_lock_release`, leaving `state.lock` behind and stalling every later write on the 5s acquire timeout. `ac_lock_acquire` now registers the held lock in `AC_HELD_LOCK` and the funnel releases it on either refusal branch, so lock cleanup is a property of the write guard itself rather than of each caller's error path; all twelve locked callers are covered uniformly.
...
- `test-promotion.sh` additionally pins, through `bin/arc`: an instinct-only fingerprint promoting with the observation's text; the candidate, idempotent, and arbitrary-basis paths; missing and corrupt `state.json` diagnosed as a state problem rather than a fingerprint miss; and duplicate-fingerprint candidates promoting once. `test-state.sh` pins both funnel-refusal branches under the lock: no `state.lock` remains and the next write succeeds without the acquire stall.
