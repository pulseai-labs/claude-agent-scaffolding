...
```
Each record carries: `run_id, host_agent, adversary, artifact_path, depth, status, started_at, completed_at, result_path, codex_session_id, neutral_mode, walk_mode, resolved_run_request_id`. The `target_root` for spine calls is `"$arc_bin" codex_target_root "<artifact_path>"`.
...
1. Read the record: `"$arc_bin" state_external_run_get "<run-id>"` (rc1 → tell the user no such run; list runs).
2. If `status == running`, check once for a fresh disposition (non-blocking and non-mutating — do **not** use the bounded wait loop):
   ```bash
   term="$("$arc_bin" codex_status "$("$arc_bin" codex_target_root "<artifact_path>")" "<run-id>")"
   ```
   Update the stored status only if it changed to a terminal token: `"$arc_bin" state_external_run_set_status "<run-id>" "$term"` for `completed|failed|cancelled|stalled|capped`. Never persist `running` or `error`.
...
3. Fetch the raw Codex challenges (no rebuttal): `"$arc_bin" codex_result "$("$arc_bin" codex_target_root "<artifact_path>")" "<run-id>"` → a `{challenges,gaps}` object. If this fails, set the external run to `failed`, report that the stored Codex result is malformed/unparseable, and stop without entering consolidation.
...
2. Cancel via the spine and record the returned disposition:
   ```bash
   term="$("$arc_bin" codex_cancel "$("$arc_bin" codex_target_root "<artifact_path>")" "<run-id>")"
   ```
   If `term == cancelled`, persist `"$arc_bin" state_external_run_set_status "<run-id>" cancelled`. If `term == completed`, persist completed and tell the user the job completed before cancellation, so `resume` is still available. For `failed|stalled|capped`, persist that terminal token; for `error`, report the error without overwriting state.
...
6. **Append + mark resolved atomically.** After the rebuttal concludes, mint the run's `request_id` and finalize with one locked state transaction:
   ```bash
   "$arc_bin" state_external_run_finalize_resume \
     --run-id "<run-id>" \
