...
only after an immutable bundle tag is published. `code-judo`, `orca-crew` and
`herdr-crew` are not in the bundle at all — and because an unrecognised name in the plugin allowlist makes the
...
`claude-security-audit`, `orca-crew`, and `herdr-crew` are not published to Devin. Skills
...
├── ossify/                            # skeleton-first replacement lifecycle (v1.12.0)
├── code-judo/                         # code-judo plugin (v0.1.0)
├── orca-crew/                         # orca-crew plugin (v0.7.0)
├── herdr-crew/                        # herdr-crew plugin (v0.1.0)
├── docs/
