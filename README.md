...
`herdr-crew`, `dsh-crew` and `merge-bar` are not in the bundle at all — and because an unrecognised name in the plugin allowlist makes the
...
`claude-security-audit`, `orca-crew`, `herdr-crew`, `dsh-crew`, and `merge-bar` are not published to Devin. Skills
