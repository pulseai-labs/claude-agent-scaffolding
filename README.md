...
| [`dsh-crew`](./dsh-crew/) | v0.1.0 | User-level | The crew for DeepSeek Harness (dsh): a spine session on the `crew-spine` preset runs `ossify`'s `run-spine <spine-id> --external-executor`, sends each work item to a configured `subagent_implementer` child, verifies it with a `subagent_verifier` child, allows one correction on FAIL, and hands the round back as ossify's result records computed from git. 2 prose skills and a presets reference, no runtime library, no commands. Composes with `ossify` through `run-spine --external-executor` and adds nothing to ossify's contract; ossify state stays the single authority. |
